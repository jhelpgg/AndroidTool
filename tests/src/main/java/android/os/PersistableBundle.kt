...
class PersistableBundle(private var classLoader: ClassLoader?) : Parcelable
{
   companion object CREATOR : Creator<PersistableBundle>
   {
      @JvmStatic
      val EMPTY = PersistableBundle()

      override fun createFromParcel(source: Parcel): PersistableBundle
      {
      ...
      override fun newArray(size: Int): Array<PersistableBundle?>
      {
   ...
   constructor(bundle: PersistableBundle) : this(null)
   {
   ...
   fun clone() = PersistableBundle(this)

   fun deepCopy() = PersistableBundle(this)
